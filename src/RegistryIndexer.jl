...
function init_package_info!(pkg::PkgEntry)
    ...
    for (v, data) in compat_data_toml
        ...
        d = Dict{String,Version.VersionSpec}(dep => Versions.VersionSpec(vr_dep) for (dep, vr_dep) in data)
        compat[vr] = d
    end
    ...
    deps = Dict{Versions.Versions.VersionRange,Dict{String,UUID}}()
    for (v, data) in deps_data_toml
        vr = Versions.Versions.VersionRange(v)
        d = Dict{String,UUID}(dep => UUID(uuid) for (dep, uuid) in data)
        deps[vr] = d
    end
    # All packages depend on julia
    deps[Versions.Versions.VersionRange()] = Dict("julia" => JULIA_UUID)
